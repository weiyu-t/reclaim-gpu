...
export function NodeDecisions({ price, briefing }: { price: number; briefing: (evidenceId?: string) => React.ReactNode }) {
  const { data, status } = useEvidence('/node-audit?price=' + price)
  const [selected, setSelected] = useState(0)
  const [raw, setRaw] = useState<string | null>(null)
  if (!data) return status
  const c = data.cases[Math.min(selected, data.cases.length - 1)]
  const h = c?.hardware
  ...
  return <>
    {status}
    <div className="plan-summary"><ShieldCheck size={24} /><div><strong>Review the proposed machine removals before approving them.</strong><p>{data.baseline.nodes.length ? `The proposed API ranks ${data.baseline.nodes.length} machines. Its estimated value is ${usd(data.baseline.recommendation?.estimated_savings.amount || 0)}; that estimate is not verified savings.` : 'No machines are ranked by the supplied findings.'} {data.baseline.includes_hardware_node === false ? 'Its ranking omits the strongest raw-supported machine episode.' : data.baseline.includes_hardware_node === true ? 'Its ranking includes the strongest raw-supported episode; each removal still needs its own evidence.' : 'No machine episode meets the inspection thresholds.'}</p></div><span className="pill neutral">Layer B audited</span></div>
    {data.rejected_hardware_findings.length > 0 && <p className="inline-error">{data.rejected_hardware_findings.length} supplied hardware findings could not be validated against the current records. Their verdicts are not used.</p>}
    {data.cases.length > 0 ? <label className="planner-field"><span>Investigated node windows ({data.cases.length})</span><select aria-label="Investigated node window" value={Math.min(selected, data.cases.length - 1)} onChange={e => setSelected(+e.target.value)}>{data.cases.map((x: any, i: number) => <option key={x.id} value={i}>{x.node} · {date(x.start)} · {labels[x.cause]}</option>)}</select></label> : <section className="card research-card"><h2>No node windows to investigate</h2><p>The current elevated-failure screen and supplied findings identify no candidate windows. This is not proof that all machines are healthy.</p></section>}
    {c && <>
    <section className="card research-card" role="tabpanel" aria-label={labels[c.cause]}>
      ...
      {c.cause === 'cannot_determine' && <div className="research-callout"><strong>The cause remains unresolved.</strong><p>The raw controls do not establish a machine or shared-workload cause in this window. A finding or a past fault does not justify carrying a diagnosis forward.</p></div>}
    ...
    {h && <DrainLab key={h.evidence_id} price={price} episode={h} defaultWidth={data.gpus_per_node} />}
    </>}
    <section className="card research-card"><div className="section-top"><div><span className="eyebrow">CHECK THE RECOMMENDATION</span><h2>Why the automatic recommendation needs review</h2></div><Cpu size={22} /></div><p className="research-body">{data.baseline.audit}</p>
      ...
      {(h || !data.hardware) && briefing(h?.evidence_id)}
    </section>
    <section className="card quiet-card"><ShieldCheck size={26} /><div><h3>{data.history.jobs} jobs encountered {data.history.attempts} scheduler-recorded failed attempts.</h3><p>Only {data.history.terminal_node_fail} finish as NODE_FAIL. A final outcome hides retry history. Our hardware-count claim uses recorded scheduler evidence and keeps machine-signature investigations separate.</p></div></section>
    {raw && <RawJob id={raw} close={() => setRaw(null)} />}
  </>
}

function DrainLab({ price, episode, defaultWidth }: { price: number; episode: any; defaultWidth: number }) {
  const [duration, setDuration] = useState(4)
  const [fraction, setFraction] = useState(50)
  const [operator, setOperator] = useState(1)
  const [nodes, setNodes] = useState(1)
  const [width, setWidth] = useState(defaultWidth)
  const { data, status, pending } = useEvidence(`/drain-scenario?price=${price}&duration=${duration}&recurrence=${fraction / 100}&operator_hours=${operator}&nodes=${nodes}&gpus_per_node=${width}&evidence_id=${encodeURIComponent(episode.evidence_id)}`)
  return <section className="card research-card"><div className="section-top"><div><span className="eyebrow">COST OF TAKING MACHINES OUT OF SERVICE</span><h2>What does taking capacity away cost?</h2></div><FlaskConical size={23} /></div>
    <p className="research-body">Reference episode: {episode.node}, {date(episode.start)}–{date(episode.end)}. Assume one repeat; price avoidable GPU time against unavailable capacity and inspection effort. Research disruption and queue delays still need an owner’s assessment. The machine count and GPU width are assumptions to verify against inventory; they do not reproduce the proposed API’s selection.</p>
    <div className="drain-grid"><div className="drain-controls">
      ...
      <label className="planner-field"><span>Machines made unavailable</span><input aria-label="Machines made unavailable" type="number" min={1} max={10000} value={nodes} onChange={e => setNodes(Math.min(10000, Math.max(1, Math.trunc(+e.target.value))))} /></label>
      <label className="planner-field"><span>GPUs per machine (assumed)</span><input aria-label="GPUs per machine" type="number" min={1} max={1024} value={width} onChange={e => setWidth(Math.min(1024, Math.max(1, Math.trunc(+e.target.value))))} /></label>
    </div><div className="drain-result" aria-busy={pending}>{status}{pending && data && <span className="small-muted">Updating scenario…</span>}{data && !data.available && <p>{data.reason}</p>}{data?.available && <>
...
export function GPUCards({ price }: { price: number }) {
  ...
  return <>
    {status}
    <div className="plan-summary"><Cpu size={24} /><div><strong>{data.total ? 'Some jobs may be able to use fewer GPUs.' : 'No supported GPU-allocation trial in this screen.'}</strong><p>Test the GPU allocation with workload owners. These estimates use completed jobs without retries and cap GPU hours to the job’s duration.</p></div><span className="pill neutral">Not in recovery total</span></div>
    ...
    {example ? <section className="card research-card"><div className="section-top"><div><span className="eyebrow">INDIVIDUAL GPU USE</span><h2>Which GPU did the work?</h2></div><span className="pill green">Measured per card</span></div>
    ...
    : <section className="card research-card"><h2>No matching GPU allocations</h2><p>No jobs meet the current imbalance and evidence thresholds.</p></section>}
    <div className="research-two-col"><section className="card research-card"><span className="eyebrow">MEMORY STILL MATTERS</span><h2>{data.memory_above_one_percent_cards} quiet-card allocations exceeded 1% peak memory.</h2><p className="research-body">The strict screen measures {whole(data.thresholds[0].gpu_hours)} GPU-hours. PCIe counters can be coarse or censored; they do not establish sustained useful transfers. Verify output parity and runtime before reducing the allocation.</p></section>
      <section className="card research-card"><span className="eyebrow">PLACEMENT ASSOCIATION</span><h2>Check how jobs assign work to GPUs</h2><p className="research-body">{Object.entries(data.quiet_card_index).map(([index, count]) => `Local GPU ${index}: ${count} quiet rows. `).join('') || 'No quiet-card rows. '} {data.index_caveat}</p></section></div>
