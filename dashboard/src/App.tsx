...
type Overview = {
  dataset: { name: string; revision: string; epoch_offset: number; gpus_per_node: number }
  decision: { title: string; description: string; action_count: number }
  pcie_findings: number
...
function App() {
  ...
  async function reloadDataset() {
    ...
    try {
      const r = await fetch('/api/reclaim/dataset/reload', { method: 'POST' })
      const result = await r.json()
      if (!r.ok) throw new Error(result.detail || 'Dataset reload failed.')
      const fresh = await get<Overview>('/overview?price=' + price)
      setSelected(null); setData(fresh); setRevision(r => r + 1)
    } catch (e) { setError(String(e)) } finally { setReloading(false) }
  }
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Main navigation">
        {nav.map(n => <button key={n.id} aria-label={n.label} aria-current={tab === n.id ? 'page' : undefined} title={n.label} className={'nav-item ' + (tab === n.id ? 'active' : '')} onClick={() => navigate(n.id)}>
          <n.icon size={18} /><span>{n.label}</span>{n.id === 'plan' && <span className="nav-count">{data?.decision.action_count ?? 0}</span>}
        </button>)}
      </nav>
      <div className="sidebar-spacer" />
      <div className="source-card"><div className="source-icon"><Database size={17} /></div>
        <strong>{data?.dataset.name || 'Local dataset'}</strong><span>Loaded workload snapshot</span>
        <div className="source-status"><i />{data ? 'Snapshot ' + data.dataset.revision.slice(0, 8) : 'Loading'}</div>
    ...
    <div className="main-shell">
      <header className="topbar">
        ...
        <div className="topbar-actions"><button className="button secondary" onClick={reloadDataset} disabled={reloading}>{reloading ? 'Loading…' : 'Reload data'}</button>
      ...
      <main key={data?.dataset.revision}>
        {!data ? <div className="loading-state"><span className="logo"><ArrowUpRight /></span><h1>{error ? 'The data service is unavailable.' : 'Loading the budget review…'}</h1>
          ...
          {tab === 'overview' && <>
            <section className="executive-brief" aria-label="Budget decision brief">
              <div className="executive-recommendation"><div><span className="eyebrow">RECOMMENDATION</span><h2>{data.decision.title}</h2><p>{data.decision.description}</p></div><span className="executive-status">{data.decision.action_count ? 'Proposed · not yet tested' : 'No eligible workloads'}</span></div>
              ...
              <div className="executive-footer"><p>Historical sample at ${data.price.usd_per_gpu_hour.toFixed(2)}/GPU-hour. Trial results are unproven; actual recovery could be zero.</p><button className="button mint" disabled={!data.decision.action_count} onClick={() => navigate('planner')}>Plan a trial<ArrowUpRight size={16} /></button></div>
            </section>
            ...
            <div className="actions-grid">{!data.actions.length && <p className="small-muted">{data.decision.description}</p>}{data.actions.map((a, i) => <ActionCard key={a.id} action={a} index={i} onOpen={() => setSelected(a.id)} />)}</div>
          ...
          {tab === 'plan' && <>
            <div className="plan-summary"><ShieldCheck size={23} /><div><strong>{data.decision.title}</strong><p>The starting estimate is {whole(data.recovery.point)} GPU-hours freed. Confirm the results and runtime before changing normal operations.</p></div><span className="pill green">{pct(data.recovery.share_percent)} of sample</span></div>
            <div className="actions-grid">{!data.actions.length && <p className="small-muted">{data.decision.description}</p>}{data.actions.map((a, i) => <ActionCard key={a.id} action={a} index={i} onOpen={() => setSelected(a.id)} expanded />)}</div>
          ...
          {tab === 'planner' && <TrialPlanner price={price} datasetRevision={data.dataset.revision} />}
          {tab === 'nodes' && <NodeDecisions price={price} briefing={evidenceId => <NodeBriefing key={String(price) + evidenceId} price={price} evidenceId={evidenceId} />} />}
          ...
          <footer className="main-footer"><span><Database size={13} />{data.dataset.name} · observed workload sample</span><button onClick={() => navigate('method')}>Capacity value ≠ cash savings <ArrowUpRight size={13} /></button></footer>
        </>}
      </main>
    </div>
...
function Spend({ data }: { data: Overview }) {
  const max = Math.max(1, ...data.weekly.map(w => w.gpu_hours))
  return <section className="card spend-card">
    ...
    <div className="spend-layout">
      ...
      })}</svg><div><span>{data.weekly[0]?.week || 'No observations'}</span><span>{data.weekly.at(-1)?.week}</span></div></div>
    </div>
  ...
  return <div className="risk-grid">
    ...
    <div className="risk-results">{error && <p className="inline-error">{error}</p>}
      <section className={'risk-answer ' + ((scenario?.net_capacity_value_usd || 0) < 0 ? 'negative' : '')}>
        ...
        <p>{scenario && scenario.capacity_value_usd === 0 ? 'No recovery is assumed or eligible. These figures do not justify an intervention.' : scenario && scenario.net_capacity_value_usd < 0 ? 'Rework costs exceed the value of freed GPU time. Reduce disruption before expanding the trial.' : 'The value of freed GPU time exceeds rework costs under these assumptions. Confirm both in a trial.'}</p>
...
function EvidencePage({ data, price, onOpen }: { data: Overview; price: number; onOpen: (id: string) => void }) {
  ...
  useEffect(() => { get('/causal-case').then(setCaseData).catch(e => setError(e.message)).finally(() => setCaseLoaded(true)) }, [])
  ...
  return <>
    ...
    <section className="card causal-card"><div className="section-top"><div><span className="eyebrow">SHARED WORKLOAD INVESTIGATION</span><h2>{caseData?.decision || 'Check for shared workload failures'}</h2></div><span className="pill green">Real telemetry</span></div>
      <p className="section-description">Arrays are screened from the loaded job records. A shared failure is an investigation lead; it does not establish a physical or software diagnosis.</p>
      {error && <p className="inline-error">{error}</p>}
      {!caseLoaded ? <LoaderCircle className="spin" /> : !caseData ? <p>No array meets the failure-screening criteria in this dataset.</p> : <>
        <div className="causal-grid"><div className="causal-copy"><div className="causal-numbers"><div><strong>{whole(caseData.tasks)}</strong><span>failed tasks</span></div><div><strong>{caseData.nodes}</strong><span>machines involved</span></div><div><strong>1</strong><span>shared array</span></div></div>
          ...
          <div className="causal-graph"><div className="graph-root"><Layers3 size={24} /><span>Shared workload<strong>{caseData.root_name}</strong></span></div><div className="graph-stem" /><div className="graph-nodes">{caseData.node_counts.slice(0, 4).map((n: any) => <div key={n.node}><Cpu size={20} /><strong>{n.jobs} tasks</strong><span>{n.node}</span></div>)}</div><p>{caseData.replicated ? 'Matching failures span machines; check the common workload.' : 'A replicated cross-machine cause has not been established.'}</p></div>
        </div>
    ...
    <section className="card quiet-card"><ShieldCheck size={28} /><div><h3>{data.pcie_findings ? 'Review PCIe findings before proposing an upgrade' : 'No PCIe saturation findings in this snapshot'}</h3><p>{data.pcie_findings} supplied findings. Validate current measurements before attributing a bottleneck or approving more capacity. Missing findings do not establish that the rule was run.</p></div><span className="pill green">{data.pcie_findings ? 'Review findings' : 'No findings supplied'}</span></section>
  </>
}
...
function Briefing({ brief }: { brief: Brief }) {
  return <div className="briefing"><div className="section-top"><h3><Sparkles size={17} />Evidence briefing</h3><span className={'pill ' + (brief.mode === 'live' ? 'green' : 'neutral')}>{brief.mode === 'live' ? 'Featherless + MCP' : 'MCP evidence · no model'}</span></div>
    ...
    <details><summary>{brief.tool_trace.length} MCP tool calls · {brief.elapsed_seconds}s{brief.cached ? ' · cached' : ''}{brief.usage?.total_tokens ? ' · ' + whole(brief.usage.total_tokens) + ' tokens' : ''}</summary>
      {brief.tool_trace.map((t, i) => <div className="tool-call" key={i}>{t.status === 'ok' ? <Check size={14} /> : <CircleHelp size={14} />}<code>{t.tool}</code>{t.status !== 'ok' && <span>{t.status}</span>}<span>{t.elapsed_ms} ms</span></div>)}
      {brief.model && <p className="small-muted">Model: {brief.model}</p>}
    </details>
  </div>
}

function NodeBriefing({ price, evidenceId }: { price: number; evidenceId?: string }) {
  const [brief, setBrief] = useState<Brief | null>(null)
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState('')
  async function explain() {
    setBusy(true); setError('')
    try {
      const r = await fetch('/api/reclaim/investigate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action_id: 'node-audit', price, evidence_id: evidenceId }) })
      if (!r.ok) throw new Error('The node briefing could not complete. Please retry.')
      setBrief(await r.json())
    } catch (e) { setError(String(e)) } finally { setBusy(false) }
  }
  return <div className="node-brief"><button className="button primary" onClick={explain} disabled={busy}>{busy ? <LoaderCircle className="spin" size={16} /> : <Sparkles size={16} />}{busy ? 'Retrieving evidence…' : 'Explain the machine recommendation'}</button><p className="small-muted research-note">This checks the selected episode when one is supported, using the starting cost assumptions. It does not update with the sliders. With insufficient evidence, it explains why no action is supported.</p>{error && <p className="inline-error">{error}</p>}{brief && <Briefing brief={brief} />}</div>
}
    ...
    <div className="drawer-inner"><div className="drawer-top"><span className="eyebrow"><FileCheck2 size={15} />SUPPORTING RECORDS</span><button className="icon-button" onClick={onClose} aria-label="Close evidence"><X size={20} /></button></div>
      ...
      {!evidence ? <LoaderCircle className="spin" /> : <>
        ...
        <section className="drawer-section"><div className="section-top"><h3>Jobs behind the number</h3><span className="small-muted">Largest allocation first</span></div><div className="table-scroll"><table><thead><tr><th>Job ID</th><th>Outcome</th><th>GPU-h</th><th>Avg / peak SM</th><th /></tr></thead><tbody>{evidence.jobs.rows.map(j => <tr key={j.id}><td><button className="job-link" onClick={() => inspect(j.id)}>{j.id}</button></td><td><span className={'state ' + j.state.toLowerCase()}>{j.state.replaceAll('_', ' ')}</span></td><td>{whole(j.gpu_hours)}</td><td>{j.avg_util}% / {j.peak_util}%</td><td><button className="icon-button" onClick={() => inspect(j.id)} aria-label={'Inspect job ' + j.id}><ArrowUpRight size={14} /></button></td></tr>)}</tbody></table></div>
          <div className="pagination"><span>{evidence.jobs.total ? offset + 1 : 0}–{Math.min(offset + evidence.jobs.limit, evidence.jobs.total)} of {whole(evidence.jobs.total)} jobs</span><div><button className="icon-button" aria-label="Previous jobs" disabled={offset === 0} onClick={() => setOffset(o => Math.max(0, o - 25))}><ChevronLeft size={17} /></button><button className="icon-button" aria-label="Next jobs" disabled={offset + evidence.jobs.limit >= evidence.jobs.total} onClick={() => setOffset(o => o + 25)}><ChevronRight size={17} /></button></div></div>
...
function Method({ data }: { data: Overview }) {
  return <div className="method-grid">
    ...
    <section className="card"><span className="eyebrow">03 / SCOPE</span><h2>What this sample covers</h2><p>{whole(data.sample.jobs)} jobs from {data.sample.researchers} researchers ran on {data.sample.nodes} machines in this observed window. Job telemetry cannot establish total unallocated fleet idle time.</p><p>The {data.accounting.synthetic_findings} findings marked synthetic are excluded from savings. Array investigations are recomputed from job records.</p><div className="method-stat"><span>Price book</span><strong>{data.price.version}</strong></div></section>
    <section className="card"><span className="eyebrow">04 / DECISION QUALITY</span><h2>What the trials must confirm</h2><p>Low utilization is a symptom. Cancellation may preserve research resources. A node hosting failed tasks may be healthy. Findings start investigations; they do not authorize changes.</p><p>The MCP workflow retrieves rules and evidence from the loaded snapshot. Deterministic code computes every dashboard number. Model explanations are labeled separately and include their tool trace.</p><div className="method-stat"><span>Action policy</span><strong>Pilot → measure → decide</strong></div></section>
      <section className="card method-wide"><div className="section-top"><h3>Reproduce this analysis</h3><FileCheck2 size={20} /></div><p>Use the documented five-file schema for each dataset. Replace the files in the configured data directory, then click Reload data. A failed validation retains the previous snapshot. Claims are recalculated from the active snapshot. See docs/DATASETS.md for input columns, optional dataset.json metadata and loading a different directory.</p><pre>make prep{'\n'}make generate{'\n'}make check-data{'\n'}docker compose up{'\n'}make validate CLAIMS=claims.json URL=http://localhost:3000</pre><p className="small-muted">The bundled example uses MIT SuperCloud TX-GAIA (CC BY-NC-ND 4.0). Other datasets retain their own provenance and license. Telemetry is excluded from Git. Live briefings send small evidence excerpts to the configured Featherless service. Dollar amounts use a reference price, not an invoice.</p></section>
  </div>
}
