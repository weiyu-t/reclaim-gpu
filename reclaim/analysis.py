...
class Analysis:
    ...
    def actions(self, price=DEFAULT_PRICE):
        definitions = [
            {
                "id": "cpu-placement", "title": "Test CPU-only placement",
                "short_title": "CPU placement", "tag": "Placement",
                "owner": "Research platform",
                "description": "Completed jobs reserved GPUs but recorded no GPU computing. Test repeat runs on CPUs and check that results and runtime are acceptable.",
                ...
                "pilot": "Invite owners of a few repeat jobs to try CPUs. Check that results match and completion time is acceptable before changing their normal setup.",
                "rollback": "Move jobs back to GPUs if results change, runs become materially slower, or waits for CPUs increase.",
            },
            {
                "id": "idle-sessions", "title": "Trial low-activity session warnings",
                "short_title": "Idle-session policy", "tag": "Scheduling",
                "owner": "Scheduler operations",
                "description": "Long GPU sessions recorded little activity. Warn owners first; measure whether sessions can safely end before adding a timeout.",
