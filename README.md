...
Answers must match a four-field JSON structure and cite a retrieved finding ID. The model supplies recommendation, downside and pilot prose; the numerical evidence section is rendered directly from the calculation code. Model prose containing digits is rejected. Prose still needs review against the records; these checks do not prove its correctness. Provider reasoning, request headers and keys never enter the run log. Each model attempt has a 45-second total deadline. Container usage logs live in `/app/out/agent_runs.jsonl`; native runs write `out/agent_runs.jsonl`. Logs are ignored by Git and are not persistent across container replacement.
